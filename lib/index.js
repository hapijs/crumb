...
internals.defaults = {
    ...
    method: 'hmac',               // Define the token generation (and therefore the validation) method
    sessionKey: 'userId'            // Define which key of auth.credentials should be used during token creation
};
...
exports.plugin = {
    ...
    register: function (server, options) {
        ...
        server.ext('onPostAuth', (request, h) => {
            ...
            const unauthorizedLogger = (debugTags) => {

                if (settings.logUnauthorized) {
                    let tags = ['crumb', 'unauthorized'];
            ...
            if (restful) {
                ...
                if (settings.method === 'hmac' && !Hmac.validate(header, request.auth.credentials[settings.sessionKey], settings.secret)) {
                    unauthorizedLogger(['restful', 'hmac']);
                    throw Boom.forbidden();
                }
            ...
            if (!content ||
                ...
                unauthorizedLogger(['no_content_or_stream']);
                throw Boom.forbidden();
            }
       ...
       if (settings.method === 'hmac' &&
                ...
                unauthorizedLogger(['crumb_value']);
                throw Boom.forbidden();
            }
            // Remove crumb
