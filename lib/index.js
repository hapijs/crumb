...
internals.routeDefaults = {
    name: 'crumb',                   // query or payload key
...
exports.register = function (plugin, options, next) {
    ...
    // copy the name from internals.defaults to internals.routeDefaults for consistency
    internals.routeDefaults.name = settings.name;
    ...
    plugin.ext('onPostAuth', function (request, next) {
        ...
        if (content[request.route.plugins._crumb.name] !== request.plugins.crumb) {
            return next(plugin.hapi.error.forbidden());
        }

        // Remove crumb

        delete request[request.route.plugins._crumb.source][request.route.plugins._crumb.name];
    ...
    plugin.ext('onPreResponse', function (request, next) {
        ...
        if (settings.addToViewContext &&
            ...
            response.view.context[request.route.plugins._crumb.name] = request.plugins.crumb;
        }
